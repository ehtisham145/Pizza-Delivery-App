...
@delivery_router.post("/add_new_address/customer",status_code=status.HTTP_201_CREATED,response_model=AddressResponseSchema)
def add_new_address(address:AddressCreateSchema,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    #1.Check Address Length
    count_addresses=db.query(func.count(Delivery_Model.id)).filter(Delivery_Model.user_id==user.id).scalar()
    #2.Raise Error
    if count_addresses>=5:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,detail="Maximum 5 Addresses are Allowed Per User !")
...
@delivery_router.get("/get_all_addresses/Customer",status_code=status.HTTP_200_OK)
def get_all_addresses(db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    #1.Check whether user has add address in db or not
    valid_user=db.query(Delivery_Model).filter(Delivery_Model.user_id==user.id).first()
    #2.Raise Error
    if not valid_user:
...
@delivery_router.patch("/Set_Deafult_Address/customer/{address_id}",status_code=status.HTTP_200_OK)
def set_deafult_address(address_id:int,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
...
@delivery_router.patch("/Update_Address/customer/{address_id}",status_code=status.HTTP_200_OK,response_model=AddressResponseSchema)
def update_address(
    address_id:int,
...
@delivery_router.delete("/Delete_Address/customer/{address_id}",status_code=status.HTTP_204_NO_CONTENT)
def delete_address(
    address_id:int,
    db:Session=Depends(get_db),
    user:User=Depends(get_current_user)
